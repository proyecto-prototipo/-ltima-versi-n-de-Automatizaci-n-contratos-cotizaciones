# -ltima-versi-n-de-Automatizaci-n-contratos-cotizaciones
/*****************************************************
...
function norm(s) { 
  return String(s || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
    .replace(/[^\w\s%]/g, "") // Permite el símbolo % para descuentos
    .replace(/\s+/g, "_").trim(); 
}
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]) === String(id)) {
      ...
      headers.forEach((h, j) => {
        ...
        // --- CORRECCIÓN PRIORIDAD PORCENTAJE (30%) ---
        if (key.includes("%")) {
          val = (Number(val || 0) * 100).toFixed(0) + "%";
        } 
        // Solo si NO es porcentaje y NO es num_cuotas, formateamos como dinero
...
// ================== FUNCIÓN ÚNICA: GENERAR CONTRATO ==================
...
function generarContratoDesdeCotizacion_BOTON() {
  ...
  try {
    ...
    setByHeader(sheet, map, row, "fecha_contrato", fechaRegistro); // Columna AI
    setByHeader(sheet, map, row, "link_contrato", urlContrato);   // Columna AJ
...
/**
 * Función interna unificada (Emojis limpios, Soles correctos y Firmas)
 */
function _ejecutarCreacionDocumento(sheet, map, row, ruc, cliente, fechaObjeto) {
  ...
  headers.forEach((h, i) => {
    ...
    // --- CORRECCIÓN PORCENTAJE EN CONTRATO ---
    if (key.includes("%")) {
...
function _formatSoles_(val) {
  ...
  // Formato US para asegurar COMA en miles y PUNTO en decimales
